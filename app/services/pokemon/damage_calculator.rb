  ...
  class DamageCalculator
    ...
    class << self
      ...
      def calculate(attacker:, defender:, move:)
        ...
        if effectiveness == 0
          return { min: 0, max: 0, stab: stab?(move_record.move_type, attacker_base.types),
                   effectiveness: 0.0, attacker_stat: atk_stat, defender_stat: def_stat }
        end
      ...
      def resolve_move(move)
        return move if move.is_a?(Pokemon::Move)
        Pokemon::Move.find_by!(name: move[:name])
      end
      ...
      def zero_result
        { min: 0, max: 0, stab: false, effectiveness: 0.0, attacker_stat: 0, defender_stat: 0 }
      end
    end
  end
end
