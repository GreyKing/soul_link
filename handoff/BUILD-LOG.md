...
**Active step:** Step 19 — Move-name lookup + Discord notifier + Wipe check. Three "events that happen during a run get observed and surfaced" features bundled into one step. **Reviewed by Richard: 0 Must Fix, 0 Should Fix, 2 Nice-to-Have (rescue list redundancy + view-var inconsistency, both noted, no change). 1 Note escalated to Architect (map-view NEW CATCH form gating) — folded in for read-only consistency.** KG-17 closes (Discord notifier `notify_run_complete` fires on HoF). KG-24 closes (move-name lookup ships).

**Test count:** 596 → **654** (+58). 0 failures, 0 errors. 2011 assertions.
**Lint:** rubocop clean (184 → **191** files, 0 offenses).
**Brakeman:** Clean (no new warnings; 2 pre-existing weak-confidence warnings on `emulator_controller.rb:79` SendFile + `gym_schedule_discord_update_job.rb:14` FileAccess unchanged).
**Migrations:** 1 new (additive, nullable); ran cleanly on dev + test DBs.

**Shipped:** Step 19 committed at `435a1c9` on branch `claude/pedantic-jemison-e7d667`, FF-merged to `origin/main` and pushed.
**Pending deploy:** Step 19 ships 1 migration (`add_wiped_at_to_soul_link_runs` adds `wiped_at datetime` nullable, no index). Pre-deploy safe — additive only, no data backfill, no breaking changes. Existing rows default to nil → `wiped?` returns false → `read_only?` returns false → no UI changes for in-flight runs.

**Richard's verifications (notable):**
- WebFetched PKHeX `text_Moves_en.txt` directly and cross-checked five boundary IDs (1=Pound, 33=Tackle, 165=Struggle, 257=Heat Wave, 467=Shadow Force) plus three Gen-IV-only spot-checks (354=Psycho Boost, 400=Night Slash, 428=Zen Headbutt). All match `config/soul_link/move_names.yml`. 467 entries, no gaps, IDs 1..467 contiguous. Bob's off-by-one resolution (extracting source lines 2..468 because line 1 is the no-move sentinel) is correct.
- Traced `WipeCoordinator` idempotency: outer `return if run.wiped_at.present?` guard + inner double-check inside `with_lock`. Ruby `do/end` block return semantics confirmed — returns from enclosing `process` method; `notify_wipe` is unreachable on idempotent path.
- Wipe fan-out: 4 partner-linked deaths via `mark_as_dead!` → 4 `notify_death` (deaths channel) + 1 `notify_wipe` (general channel). Verified in controller code path.
- Schema diff against `origin/main` confirmed clean: only the version bump + `wiped_at` column. Bob's manual restore of the pre-existing FK line preserved.
- Brakeman warnings confirmed pre-existing — no diff against origin/main on `emulator_controller.rb` or `gym_schedule_discord_update_job.rb`.
- All Step-19 tests use FactoryBot factories. (a)..(e) wipe scenarios + `was_marked` precondition + off-feed flag all have dedicated tests, not just side-effect inferences.

**Nice-to-Have items accepted as-is (no code change):**
- **Discord notifier rescue list redundancy.** `StandardError` is the superclass of every other listed exception. Bob commented as "belt-and-suspenders" — keeps the failure-mode documentation on-site. Cosmetic only; future cleanup can consolidate.
- **`active_run` vs. `@run` view-var inconsistency.** `_runs_content.html.erb` uses `active_run`; the other partials use `@run`. When viewing a past wiped run via `?run_id=N`, the buttons would hide on the dashboard but the controllers all server-resolve to `current_run` — UI hide-only is the v1 contract; server enforcement is logged as KG-28.

**Decisions folded in during the deploy gate:**
- **Map-view NEW CATCH form gated for read-only consistency.** Brief decision 11's affordance list named dashboard surfaces; Richard escalated `app/views/map/show.html.erb:228` (`timeline_controller#submitCatch` form) as a parallel surface that wasn't gated. Two-line edit added (`unless dashboard_read_only?(@run)`) — `MapController#show` already exposes `@run`. UX consistency: read-only must propagate to all catch surfaces.

**Project review:** `handoff/PROJECT-REVIEW-2026-04-30.md` — KG-7 (real-save offset verification for `MAP_ID_OFFSET`) still open. KG-25 (real-SRAM smoke test for `BoxParser` + `PkmDecoder` field reads) still open. New gaps logged below (KG-26, KG-27, KG-28, KG-29).
