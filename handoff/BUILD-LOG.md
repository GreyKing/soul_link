...
**Active step:** Step 17 — PkmDecoder + PartyParser + catches+routes auto-detection. **Reviewed + cleared by Richard (0 Must Fix, 0 Should Fix).** KG-11 closed (party block offset `0xA0` pinned to PKHeX `SAV4Pt.cs` `GetSAVOffsets()` `Party = 0xA0`, cross-checked against pret/pokeplatinum `include/party.h`). KG-12 closed (Sinnoh met-location enum sourced from PKHeX `text_hgss_00000_en.txt` + `Locations.cs`; 127 entries spanning routes 201-230, all gym towns, key dungeons + 5 special event pseudo-IDs flagged `event: true`). Ready to FF-merge to main + push.

**Test count:** 461 → **527** (+66). 0 failures, 0 errors. 1682 assertions.
**Lint:** rubocop clean (169 → **178** files, 0 offenses).
...
- WebFetched PKHeX `PokeCrypto.cs` `BlockPosition` table — every one of the 24 4-tuples matches Bob's `SHUFFLE_TABLE` (cases 0..23). The mirroring of cases 24-31 to cases 0-7 follows PKHeX's "duplicates appended to eliminate modulus" pattern.
- WebFetched PKHeX `SAV4Pt.cs` — `Party = 0xA0` confirmed alongside `Trainer1 = 0x68`, `Extra = 0x2820`. KG-11 closure is real.
- WebFetched PKHeX `text_hgss_00000_en.txt` — IDs 0/16/48/50/84/117 all match `met_locations.yml` exactly. KG-12 closure is real.
- Confirmed `SoulLinkPokemon` validations remain back-compat: `species` and per-group `discord_user_id`-uniqueness are both gated on `soul_link_pokemon_group_id.present?`, so Step-17 rows (group_id: nil) skip them.
- Confirmed retry-safety pattern (`update_columns(parsed_party_data:)` BEFORE dispatch) — mirrored regression test exists in `parse_save_data_job_test.rb:526-567`.
...
### Closed in Steps 9-17 (2026-04-30 → 2026-05-03)
...
### SRAM auto-tracking (locked 2026-05-02, extended 2026-05-03 + 2026-05-03 Step 17)
