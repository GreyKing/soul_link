...
**Active step:** Step 18 — Per-Pokémon stats (Nature / IVs / EVs / moves) + PC box parsing. **Reviewed by Richard: 1 Must Fix on storage CRC range, fixed inline.** KG-21 closes (PC box parsing now ships; storage block at partition offset `0xCF2C` size `0x121E4`, box data at `+4` inside, 18 boxes × 30 slots × 136 bytes per record). Storage block has its own active-block picker (independent of general block — handles the `StorageBlockPosition` swap behavior PKHeX documents).

**Test count:** 547 → **596** (+49). 0 failures, 0 errors. 1906 assertions.
**Lint:** rubocop clean (178 → **184** files, 0 offenses).
...
**Shipped:** Step 18 committed at `132fb34` on branch `claude/lucid-lamport-624d16`. Pending FF-merge to `origin/main` + push.
**Pending deploy:** Step 18 ships 2 migrations (`add_step_18_columns_to_soul_link_pokemon` adds `ivs json` nullable, `evs json` nullable, `moves json` nullable, `caught_off_feed boolean default false null false`; `add_parsed_box_data_to_soul_link_emulator_save_slots` adds `parsed_box_data json` nullable). Pre-deploy safe — additive only, no data backfill, no breaking changes. Step-17 rows read as `caught_off_feed: false` (correct semantic — they were party-side detections by definition) and as nil for ivs/evs/moves (the view's `.present?` guards collapse the STATS expander cleanly when fields are nil). The legacy `nature` column (already on the table from earlier work) is now populated for Step-18 catches.

**Richard's verifications (notable):**
- WebFetched PKHeX `PK4.cs` and confirmed every new offset: EV_HP at 0x18..EV_SPD at 0x1D (Block A), Move1 at 0x28..Move4_PPUps at 0x37 (Block B), IV32 at 0x38 (Block B). The brief's claim that moves live in Block C was an upstream typo — Bob shipped Block B per PKHeX, correctly.
- WebFetched PKHeX `SAV4.cs` — `CalcBlockChecksum(data[..^FooterSize])` with `FooterSize = 0x14` per `SAV4Sinnoh.cs:12`. Caught a Must Fix where Bob's `STORAGE_CRC_RANGE_END` was off by 18 bytes (would have silently failed CRC on every real Platinum save). Synthetic test builder mirrored the same wrong constant, hiding it. Fix: `STORAGE_CRC_RANGE_END = STORAGE_FOOTER_OFFSET (0x121D0)`. One-line + citation comment.
- Confirmed cross-event PID dedup: same-snapshot dual-fire (party event + box event for same PID) creates exactly one row with `caught_off_feed: false` because party events process first within a single transaction and the `(run, user, pid)` `.exists?` check no-ops the box-side create.
- Confirmed pure-function discipline holds across `BoxParser` / `Natures` / extended `PkmDecoder` / extended `SaveDiff` — zero `Rails.logger`, `Time.current`, AR, file I/O. Top-level `rescue StandardError → []` (BoxParser) / `→ nil` (PkmDecoder) preserved.

**Should Fixes accepted as-is (no code change):**
- **No real-SRAM smoke test for BoxParser.** Same punt as Steps 16/17 — synthetic test builder uses the new constant, so a regression of the CRC fix would be caught only by a real-save audit. Logged as new audit follow-up KG-25.
- **`caught_off_feed: false` on Step-17 rows.** Correct semantic: Step-17 catches WERE detected via the party diff path. Acknowledged.

**Project review:** `handoff/PROJECT-REVIEW-2026-04-30.md` — KG-7 (real-save offset verification for `MAP_ID_OFFSET`) still open. KG-25 added this step (real-SRAM smoke test against a known-good Platinum dump for `BoxParser` + extended `PkmDecoder` field reads).
...
### Closed in Steps 9-18 (2026-04-30 → 2026-05-03)
...
- **KG-22: No Discord notification on auto-detected catch.** `CatchCoordinator.handle_caught` creates the row silently (the `broadcasts_refreshes_to ->(record) { [ pokemon.soul_link_run, :dashboard ] }` on `SoulLinkPokemon` does push a Turbo refresh to open dashboards in real time, so the catch is visible — just not surfaced via Discord). A 1-liner inside the coordinator could post to `run.catches_channel_id` ("New catch! GREY caught Starly on Route 201"). Deferred per brief decision 15.
...
- **KG-17: Discord notification on Hall of Fame.** Could be a 1-liner inside `HallOfFameCoordinator` (post to the run's `general_channel_id` with a "🏆 Run #N complete!" message). Deferred per brief — PO follow-on ask.
